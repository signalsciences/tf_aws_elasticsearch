...
locals {
  es_arn       = length(var.vpc_options["subnet_ids"]) > 0 ? element(concat(aws_elasticsearch_domain.es_vpc.*.arn, [""]), 0) : element(concat(aws_elasticsearch_domain.es.*.arn, [""]), 0)
  es_endpoint  = length(var.vpc_options["subnet_ids"]) > 0 ? element(concat(aws_elasticsearch_domain.es_vpc.*.endpoint, [""]), 0) : element(concat(aws_elasticsearch_domain.es.*.endpoint, [""]), 0)
  es_domain_id = length(var.vpc_options["subnet_ids"]) > 0 ? element(concat(aws_elasticsearch_domain.es_vpc.*.domain_id, [""]), 0) : element(concat(aws_elasticsearch_domain.es.*.domain_id, [""]), 0)
}
...
data "aws_iam_policy_document" "es_vpc_management_access" {
  ...
  statement {
    ...
    resources = formatlist(
      "${aws_elasticsearch_domain.es_vpc[0].arn}/%s-*/*/*",
      var.deny_del_indices_prefixes,
    )
